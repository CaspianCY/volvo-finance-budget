...
async function computeBudgetParams(year, from, to, branch) {
  ...
  const [monthlyActuals, monthlyCost, monthlyTotal, monthlyUnits, breakdown] = await Promise.all([
  ...
  const monthlyDays = dateCol
    ? await getMonthlyWorkdaysFromDate(branches, p0, p1, dateCol)
    : months.reduce((o, m) => (o[m] = weekdaysInMonth(year, m), o), {});
  ...
  return {
    ...
    basis: {
      // 透明標明台數/工作天數的實際來源
      workdays: dateCol ? `DMS 日期欄 ${dateCol}（不重複營業日）` : '月曆估算（週一~週五）',
      workdays_source: dateCol ? 'date' : 'calendar',
