 ...
 *                 營收四項與 cost 套在非實際月份上；expense（營業費用）DMS 無來源，
 *                 套在所有月份上。某月某項有值就蓋掉推估，其餘仍依推估邏輯。
 */
async function computeProjection(year, branch, method = 'target', planOverrides = {}) {
  ...
  // 成本：實際月份用 DMS；後續月份有編列用編列，否則營收×成本率
  const costOf = (m, isActual, rev) => {
    if (isActual) return monthlyCost[m] || 0;
    const ov = numOv(m, 'cost');
    return ov != null ? ov : rev * costRatio;
  };
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    const net = gross - exp;
    const costSource = isActual ? 'actual' : (numOv(m, 'cost') != null ? 'plan' : 'projected');
