...
const VALID_METHODS = new Set(['target', 'average', 'conservative']);
...
// 回傳 { m: {paid,bodywork,general,extended} }；只含財務手動編列過的月份
async function getPlan(year, branch, version = 'default') {
  const r = await pool.query(
    `SELECT period, paid, bodywork, general, extended
...
// items: [{ month|period, paid, bodywork, general, extended, note? }]
    ...
    for (const it of items) {
      ...
      await client.query(
        `INSERT INTO revenue_budget_plan
           (year, branch, version, period, paid, bodywork, general, extended, note, updated_by)
        ...
        [year, br, version, period, num(it.paid), num(it.bodywork),
         num(it.general), num(it.extended), String(it.note || ''), String(updatedBy || '')]
      );
      written++;
    }
 ...
 * @param {object} planOverrides  { m: {paid,bodywork,general,extended} } 財務編列覆寫值，
 *                 套在非實際月份上；某月某項有值就蓋掉 method 推估，其餘仍依 method。
 */
async function computeProjection(year, branch, method = 'target', planOverrides = {}) {
  ...
  const [monthlyActuals, monthlyTargets] = await Promise.all([
