...
- **營業成本**：已實現月份採 DMS 料件成本（`repair_income.parts_cost`，**僅含料件、不含工資／外包等**）；
  後續月份以「成本率 ＝ 已實現月份成本 ÷ 營收」乘該月營收推估，編列模式下可手動覆寫。
- **營業費用**：DMS 無此資料，**全部由財務手動編列**（每月皆可填，含已實現月份），未填以 0 計。
- **毛利 / 毛利率 / 營業淨利 / 淨利率**：依上述結構自動計算，逐月與全年皆顯示。
- **預算編列（版本化）**：後續月份的營收／成本與所有月份的費用可手動編輯並存成獨立版本
  （`revenue_budget_plan`），**不覆蓋**官方月目標；可載回 / 刪除 / 切換版本。實際月份的營收與成本永遠以 DMS 為準。
