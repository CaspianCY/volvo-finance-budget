...
從現有 DMS 平台的同一個 Postgres 讀取實際營收、成本與目標，把「已實現月份的實際」加上「後續月份的推估」組成全年預估，向下延伸到**材料毛利／營業淨利**，並讓財務在頁面上**編列預算、存成版本**。
...
- **材料毛利 / 材料毛利率 / 營業淨利 / 淨利率**：依上述結構自動計算，逐月與全年皆顯示。
...
| GET | `/api/budget-params?year=&branch=&from=&to=[&project=workday]` | 預算參數：從實績反推工作天數/台數/單車消費額/各項營收金額+占比/成本/材料毛利/帳類明細；帶 `project=workday` 時把 to+1~12 月以工作天數推估補成全年 |
